...
void Paquete::imprimir(int id_pag){ // colora los paquetes con id y los muestra junto con los que no lo son
	//cout<<"PAQUETE:"<<this->nro_orden<<" --> PÁGINA: "<<this->id_pagina<<endl;
	if(this->id_pagina==id_pag)
		cout<<BOLD_CYAN<<"[PAG:"<<this->id_pagina<<",Ro:"<<this->ip_comp_origen[0]<<",Rd:"<<this->ip_comp_destino[0]<<",ORD:"<<this->nro_orden<<"/"<<this->tamanio_pagina<<"]"<<ANSI_COLOR_RESET;
	else
